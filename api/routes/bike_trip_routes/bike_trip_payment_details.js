...
const Bike_Trip_Payment_Detail=require('../../models/Bike_Trip/bike_trip_payment_detail');
...
router.get('/call_back_url/:user_ns/:amount/:customerPhoneNumber/:customerEmail',(req,res)=>{
        ...
        .then(data=>{
            ...
            {
                console.log('jjjjjjj');
                const newData=new Bike_Trip_Payment_Detail({
...
router.get('/getPaymentLink',(req,res)=>{
    ...
    instance.invoices.create({
        ...
        sms_notify:1,
        email_notify:1,
        callback_url: `http://localhost:8000/admin/sub_routes/bike_trip_related_routes/bike_trip_payment_details/call_back_url/${req.body.user_ns}/${req.body.totalAmountOfTrip}/${req.body.customerPhoneNumber}/${req.body.customerEmail}`,
        callback_method: "get"
      })
      .then(response=>{
        res.json({
