...
router.get('/get_backpacking_trip_packages',(req,res)=>{
    ...
    Backpacking_Trip_Package.find({backpacking_trip_common_city_id:req.body.backpacking_trip_common_city_id,period:'new'})
    .exec()
    .then(result=>{
        res.json({
            data:result,
...
router.post('/post_backpacking_trip_package',upload.fields([{name:'front_image_path',maxCount:1},{name:'package_details_pdf_path',maxCount:1}]),(req,res)=>{

    console.log(req.files.front_image_path[0].path)
